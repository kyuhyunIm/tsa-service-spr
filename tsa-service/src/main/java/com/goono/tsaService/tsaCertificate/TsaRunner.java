...
public class TsaRunner {
	...
	public void compile() throws IOException, InterruptedException {
		...
		String cp = isWindows ? ".;JUSToolkit-1.2.13.0.jar" : ".:JUSToolkit-1.2.13.0.jar";

		String command = String.format("cd lib && javac -encoding UTF8 -cp %s GetTSAToken.java", cp);
		String[] execCommand = isWindows ? new String[] { "cmd", "/c", command }
	...
	public String run(String hash) throws IOException, InterruptedException {
		...
		String os = System.getProperty("os.name").toLowerCase();
		String[] execCommand = os.contains("win") ? new String[] { "cmd", "/c", command }
				: new String[] { "bash", "-c", command };
	...
	private String buildCommand(String hash) {
		...
		if (!isWindows) {
			envPrefix = isMac ? "DYLD_LIBRARY_PATH=$DYLD_LIBRARY_PATH:$PWD" : "LD_LIBRARY_PATH=$PWD";
		}
		...
		String baseCommand = isWindows
				...
				: String.format("cd lib && %s java %s %s GetTSAToken \"%s\" %s", envPrefix, libPathOption, cpOption,
						hash, tsaArgs);
